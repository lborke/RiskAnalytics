...
lambda_linear_func = function(Ly, Lx, Lp) {
	# uses the main algo for L1-norm Path
	fit =  qrL1(Lx, Ly, Lp, 100)
	# generalized approximate cross-validation criterion (Yuan 2006) (GACV) is used for the optimal lambda and beta coefficients:
	# the smaller the better for the minimization
	isnum0 = which(fit$Cgacv == min(fit$Cgacv))
	beta_opt = fit$beta[isnum0,] # optimal beta coefficient vector
	lambda_opt = (-fit$lambda[isnum0]) # optimal lambda value, lambda is always positive, fit delivers negative counterparts
	#print(lambda_in)
	qrL1_results = list()
	qrL1_results$lambda = lambda_opt
	qrL1_results$beta = beta_opt
	return(qrL1_results)
}
